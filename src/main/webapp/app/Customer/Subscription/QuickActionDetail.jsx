    ...
    let applyQuickAction = (action) => {
        switch(action) {
            case "activate":
                ...
                break;
            case "add_product":
                if (props?.contractId && props?.currentQueryParams?.variantId) {
                    const requestUrl = `api/v2/subscription-contracts-add-line-item?contractId=${props?.contractId}&quantity=1&variantId=gid://shopify/ProductVariant/${props?.currentQueryParams?.variantId}`;

...
export interface IOrderInfoProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const OrderInfo = (props: IOrderInfoProps) => {
  ...
  const { orderInfoList, match, totalItems } = props;
  return (
    <div>
      <h2 id="order-info-heading">
        Order Infos
      ...
      <div className="table-responsive">
        {orderInfoList && orderInfoList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('linesJson')}>
                  Lines Json <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {orderInfoList.map((orderInfo, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{orderInfo.shop}</td>
                  <td>{orderInfo.orderId}</td>
                  <td>{orderInfo.linesJson}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${orderInfo.id}`} color="info" size="sm">
          ...
          <div className="alert alert-warning">No Order Infos found</div>
        )}
      </div>
...
const mapStateToProps = ({ orderInfo }: IRootState) => ({
  orderInfoList: orderInfo.entities,

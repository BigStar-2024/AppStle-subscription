...
const PrdVariantRadioPopup = ({
  ...
  const handleCheck = (varData, imageSrc, title) => {
    ...
    const foundIndexvar = selectedItems.findIndex(({ id }) => (id === varData.id))
    if (foundIndexvar !== -1) {
  ...
  const remove = (evt, removeId) => {
    evt.preventDefault();
    setFields((prevItems) => prevItems.filter(({ id }) => id !== removeId));
  }
  ...
  return (
    <div style={{fontSize: '13px'}}>
      ...
      { fields.map(item => {
        return  <ListGroupItem className="multiselect-list-item" key={item.id} style={{padding: '0.5rem 0.6rem',marginBottom: '8px', borderRadius: '14px'}}>
                    <div>
                    <img className="swap-product-img shadow-sm mr-2 ml-1 rounded" style={{height: '48px',width: '48px'}} src={item.imageSrc}/>
                        <span className="product-id">{`#${item.id}`} &nbsp;</span>
                        <span>{item.displayName}</span>
                        <Button close onClick={(evt) => remove(evt, item.id)}></Button>
                    </div>
                </ListGroupItem>
      })
      }
                    ...
                    variants?.map((varData)=> {
                        return (
                            ...
                            <Label className="pt-2">
                                <Input type="checkbox" name="variantselect" onChange={() => handleCheck(varData, imageSrc, title)}  checked={checkIfProductAlreadySelected(varData?.id)}/>

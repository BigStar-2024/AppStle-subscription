...
const ProductUpdateMinOrderNumber = (props) => {
  ...
  const onSubmit = (values) => {
    ...
    axios.put(`/api/bulk-automations/update-min-max-cycles?updateMaxCycles=true&updateMinCycles=false${subscriptionId ? `&contractIds=${values?.subscriptionID}` :  ''}&allSubscriptions=${subscriptionId && false || allSubscriptionIds && true}${values?.newMaxOrderNumber ? `&maxCycles=${values?.newMaxOrderNumber}` : ''}`)
      .then(async res => {
                  ...
                  <Row>
                    <Col md={7}>
                        {  <FormGroup style={{marginBottom: '3px'}}>
                          <Label for="newMaxOrderNumber">Maximum Number of Orders</Label>
                          <Field
                            render={({input, meta}) => (
                              <>
                                <Input
                            ...
                            id="newMaxOrderNumber"
                            className="form-control"
                            type="text"
                            name="newMaxOrderNumber"
                            placeholder="Enter Maximum Orders Number"
                          />
                        </FormGroup>}
                      </Col>
